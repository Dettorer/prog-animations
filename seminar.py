...
def get_square_of_pred(x="x", pred_val="x-1", res="pred_x * pred_x"):
    ...
    pred_def = m.TextMobject(f"\\verb|let pred_x =|") \
        .next_to(fn_def, m.DOWN, aligned_edge=m.LEFT) \
        .shift(INDENT)
    pred_value = m.TextMobject(f"\\verb|{pred_val}|") \
        .next_to(pred_def, m.RIGHT, aligned_edge=m.UP)
    pred_end = m.TextMobject("\\verb|in|") \
        .next_to(pred_value, m.RIGHT, aligned_edge=m.UP)
    pred = m.VDict(
        ('def', pred_def),
        ('val', pred_value),
        ('end', pred_end),
    ...
    res_def = m.TextMobject(f"\\verb|{res}|") \
        .next_to(pred, m.DOWN, aligned_edge=m.LEFT)
...
class SquareOfPred(m.Scene):
    ...
    def construct_def_box(self):
        ...
        self.def_box.add(('box', m.Rectangle(height=1.5).surround(self.def_box['function'])))
    ...
    def construct_call(self, val):
        ...
        self.play(
            m.Transform(def_copy, def_instance),
            m.ShowCreation(lines),
        )
        def_instance = def_copy

        # Show context
        context = [m.TextMobject("Contexte~:", color=m.GRAY).move_to(def_instance, aligned_edge=m.LEFT)]
        ...
        self.play(m.Transform(
            context[-1],
            m.TextMobject(f"x = {val}", color=m.GRAY) \
                .next_to(context[-2], m.DOWN, aligned_edge=m.LEFT)
        ))
        ...
        # Evaluate pred_x
        def_instance_3 = get_square_of_pred(pred_val=f' {val-1} ').remove('fn').next_to(call, m.RIGHT*5)
        ...
        self.play(m.Transform(
            def_instance['res'],
            m.TextMobject(f"\\verb|{val-1} * {val-1}|").move_to(def_instance['res'], aligned_edge=m.LEFT),
        ))
        ...
        self.play(
            *[m.FadeOut(l) for l in context],
            m.Transform(call, m.TextMobject(f"\\verb|{(val-1) * (val-1)}|").move_to(call)),
